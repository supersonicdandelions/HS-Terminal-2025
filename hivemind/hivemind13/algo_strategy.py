...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_game_start(self, config):
        ...
        self.config = config
        global extra_mp
        global old_enemy_health
        global dont_spawn,oldSpawn
    ...
    def starter_strategy(self, game_state):
        ...
        global old_enemy_health
        global extra_mp
        if game_state.get_resource(MP)<20+extra_mp:
            if dont_spawn:
                if game_state.enemy_health== old_enemy_health:
                    extra_mp+=3
                else:
                    old_enemy_health=game_state.enemy_health
            old_spawn=dont_spawn
            dont_spawn=False
        ...
        if True: #game_state.turn_number % 2 == 1:
            ...
            if game_state.project_future_MP(1,0)>=20+extra_mp and not dont_spawn:
                dont_spawn=[best_location[0],13]
                game_state.attempt_remove(dont_spawn)
                go=True
            if blocked:
                leastDamage*=2
            if game_state.get_resource(MP)>=20+extra_mp: #blocked:
        ...
        if game_state.turn_number>1:
            toUpgrade=self.get_upgrade_locations(game_state,dont_spawn,old_spawn)
    ...
    def build_defences(self, game_state,dont_spawn,toUpgrade):
        ...
        currentSP= game_state.get_resource(SP)
        if all_upgraded:
            ...
            for i in [[15,12],[12,12],[17,12],[10,12],[19,12],
                      ...
                      [15,11],[12,11],[17,11],[10,11],[19,11],
                      [8,11],[21,11],[6,11]]:
                if currentSP+BASE_STRUCTURE_POINT_INCOME>=8+4+3:
                    game_state.attempt_spawn(SUPPORT, i)
                    currentSP-=4
                if currentSP+BASE_STRUCTURE_POINT_INCOME>=8+4+3:
                    game_state.attempt_upgrade(i)
    ...
    def get_upgrade_locations(self,game_state,dont_spawn,old_spawn):
        d={}
        for i in range(0,7):
            d[i]=(3,13)
        for i in range(7,14):
        ...
        for i in [[1,15],[2,16],[3,17],[4,18],[5,19],[6,20],[7,21],[8,22],
            ...
            if not game_state.contains_stationary_unit(i):
                ...
                if path[-1][1]==14:
                    ...
                    if dont_spawn[0]<7:
                        extra.add((2,13))
                    elif dont_spawn[0]>20:
                        extra.add((25,13))
                elif old_spawn and old_spawn in path:
                    locations.add(d[old_spawn[0]])
                    if old_spawn[0]<7:
                        extra.add((2,13))
                    elif old_spawn[0]>20:
                        extra.add((25,13))
        return list(locations)+list(extra)
